...
// 	Derived from FIG_1_1()	on 20070425
{
	batch init();

	init(2);
	set_gl_lines(1);
	set_window_title("FIG(1,3): Example of the use of geometric algebra");
	set_point_size(0.05);
	...
	c = c3ga_point(0.32*e1 + -2.27*e2 + 0.13*e3);
	dynamic{pl: 
		pl = shade(alpha(yellow((p.(c^ni))),0.8)),
		label(pl,"\eqn{\sigma}");
		label(RC,"\eqn{-\sigma \gp C/\sigma}");
	};
	dynamic{pnor:
		pvec = 0;
		pnor = 0;
